...
export const Globe = ({ className, config }: GlobeProps): JSX.Element => {
  ...
  const onResize = useCallback((width?: number) => {
    if (canvasRef.current) {
      setWidth(width ?? canvasRef.current.clientWidth);
    }
  }, []);
  ...
  return (
    <div
      className={cn(
        "absolute inset-0 mx-auto aspect-[1/1] w-full max-w-[100px]",
        className,
      )}
    >
      <canvas
